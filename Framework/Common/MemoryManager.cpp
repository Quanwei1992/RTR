...
int RTR::MemoryManager::Initialize()
{
...
void RTR::MemoryManager::Finalize()
{
    delete[] m_pAllocators;
    delete[] m_pBlockSizeLookup;
}

void RTR::MemoryManager::Tick()
{
}

Allocator* RTR::MemoryManager::LookUpAllocator(size_t size)
{
...
void* RTR::MemoryManager::Allocate(size_t size)
{
...
void RTR::MemoryManager::Free(void* p, size_t size)
{
    Allocator* pAlloc = LookUpAllocator(size);
    if (pAlloc)
        pAlloc->Free(p);
