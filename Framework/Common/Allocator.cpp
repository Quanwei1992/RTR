...
RTR::Allocator::Allocator(size_t data_size, size_t page_size, size_t alignment)
...
void RTR::Allocator::Reset(size_t data_size, size_t page_size, size_t alignment)
{
	FreeAll();
	...
	// because most CPU/GPU also requires the align be in 2^n
...
void RTR::Allocator::Free(void* p)
{
...
#if defined(_DEBUG)
void RTR::Allocator::FillFreePage(PageHeader *pPage)
{
	// page header
	pPage->pNext = nullptr;
...
void RTR::Allocator::FillFreeBlock(BlockHeader *pBlock)
{
	// block header + data
...
void RTR::Allocator::FillAllocatedBlock(BlockHeader *pBlock)
{
	// block header + data
...
RTR::BlockHeader* RTR::Allocator::NextBlock(BlockHeader *pBlock)
{
