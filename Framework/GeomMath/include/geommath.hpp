...
#include <cmath>
#include <iostream>
#include "CrossProduct_ispc.h"
#include "DotProduct_ispc.h"
#include "MulByElement_ispc.h"
#include "Normalize_ispc.h"
#include "Transform_ispc.h"
#include "Transpose_ispc.h"
#include "AddByElement_ispc.h"
#include "SubByElement_ispc.h"
...
namespace RTR {
	...
	struct Vector2Type
	{
		union {
			...
			struct { T r, g; };
			struct { T u, v; };
			swizzle<RTR::Vector2Type, T, 0, 1> xy;
			swizzle<RTR::Vector2Type, T, 1, 0> yx;
		};
	...
	struct Vector3Type
	{
		union {
			...
			swizzle<RTR::Vector2Type, T, 0, 1> xy;
			swizzle<RTR::Vector2Type, T, 1, 0> yx;
			swizzle<RTR::Vector2Type, T, 0, 2> xz;
			swizzle<RTR::Vector2Type, T, 2, 0> zx;
			swizzle<RTR::Vector2Type, T, 1, 2> yz;
			swizzle<RTR::Vector2Type, T, 2, 1> zy;
			swizzle<RTR::Vector3Type, T, 0, 1, 2> xyz;
			swizzle<RTR::Vector3Type, T, 1, 0, 2> yxz;
			swizzle<RTR::Vector3Type, T, 0, 2, 1> xzy;
			swizzle<RTR::Vector3Type, T, 2, 0, 1> zxy;
			swizzle<RTR::Vector3Type, T, 1, 2, 0> yzx;
			swizzle<RTR::Vector3Type, T, 2, 1, 0> zyx;
		};
	...
	struct Vector4Type
	{
		union {
			...
			swizzle<RTR::Vector3Type, T, 2, 0, 1> zxy;
			swizzle<RTR::Vector3Type, T, 2, 1, 0> zyx;
			swizzle<RTR::Vector4Type, T, 2, 1, 0, 3> bgra;
		};
